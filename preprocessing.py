...
os.chdir('C:\Projects\Veridion\workspace')
...
# 3. Apply mask: Keep valid values, replace invalid pairs with empty string ''
# np.where(condition, value_if_true, value_if_false)
clean_inputs = np.where(valid_mask, df[input_col_list], '')
clean_outputs = np.where(valid_mask, df[output_col_list], '')

# 4. Concatenate the cleaned arrays row-by-row
...
#similarity_ratio = SM(str1, str2).ratio() # 0.9032258064516129
...
#filtering the dataset by similarity ratio >= 0.8
sub_dfnonclean = df_nonclean[df_nonclean['similarity_ratio'] >= 0.8 ]
sub_dfnonclean2 = df_nonclean[df_nonclean['similarity_ratio'] >= 0.7 ]
...
clean_dataset2 = pd.concat([df_exact, sub_dfsim2], ignore_index = True)
clean_dataset2.loc[clean_dataset2['lev_ratio']==1, 'similarity_ratio'] = 1

columns_to_drop = clean_dataset2.iloc[:, np.r_[0:9, 76:83]].columns.tolist()

companies_data = clean_dataset2.drop(columns=columns_to_drop)
...
companies_data.to_csv('companies_07.csv', index=False, encoding='utf-8-sig')
